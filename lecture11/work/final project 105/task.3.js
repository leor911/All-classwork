   ...
   lifeLineUsed = false
    ...
    function displayAnswers(set){ 
        ...
        while(answerSlot <= 4) { 
            ...
            switch(answerSlot){ case 1: input = document.getElementById("answer1") 
            ...
            break; case 2: input = document.getElementById("answer2") 
            input.value = answerSet[randomIndex]  
            ...
            secondHalf = answerSet.slice(randomIndex+1) 
            answerSet = firstHalf.concat(secondHalf) 
            break; case 3: input = document.getElementById("answer3") 
            input.value = answerSet[randomIndex] 
            ...
            secondHalf = answerSet.slice(randomIndex+1) 
            answerSet = firstHalf.concat(secondHalf) 
            break; case 4: input = document.getElementById("answer4") 
            input.value = answerSet[randomIndex]
             ...
             break; } answerSlot++
                ...
                if(button.innerText === "Reset") { 
                    location.reload() } inputs = document.getElementsByTagName("input") 
                    userSelected = -1 
                    for(i = 0; i < inputs.length; i++){ 
                        if(inputs[i].checked){ userSelected = i
                             break; 
                            } 
                        } 
                        if(userSelected === -1) return 
                        if(inputs[i].value === questions[questionIndex].answer){
                             score += 200000  
                             p = document.getElementById("score") 
                             p.innerText = `${score} LLH$` } else{  
                                h3.innerText = "Lost" 
                                document.getElementById("answer1").remove() 
                                document.getElementById("answer1-label").remove() 
                                document.getElementById("answer2").remove() 
                                ...
                                document.getElementById("answer4-label").remove() 
                                document.getElementById("submit").innerText = "Reset"
                                 return } inputs[userSelected].checked = false 
                                 displayQuestion() } function alreadyUsed(index){
                                     if(usedQuestions.length === 0) return true
                                      if(usedQuestions.includes(index)) return false 
                                      else return true } 
                                      function lifeline(btn){
                                         if(btn.value > 0 && !lifeLineUsed){ 
                                            ...
                                            do{ 
                                                ...
                                                while(answers[questionIndex][randomIndex] === questions[questionIndex].answer)  p = document.getElementById("incorrect")
                                                 p.innerText = `${answers[questionIndex][randomIndex]} is not correct!` } } 
                                                 questions = [ { question: "How tall is Miles?", answer: "5ft. 5in." }, 
                                                 { question: "How many pairs of shoes does Jake own?", answer: "5" },  
                                                 { question: "What's Leo's favorite day of the week?", answer: "Thursday" },  
                                                 { question: "How many bucket hats does Cooper own?", answer: "3" }, 
                                                 { question: "What is Diego's real name?", answer: "Amaury" }, ] 
                                                 answers = [ [ "5ft. 5in.", "4ft. 11in.", "5ft. 6in.", "5ft. 9in." ], 
                                                 [ "5", "7", "11", "3" ], [ "Thursday", "Friday", "Wednesday", "Tuesday" ],
                                                  [ "3", "2", "1", "5" ], [ "Amaury", "Daniel", "Craig", "Mario" ], ] 
                                                  lifeLineUsed = false 
                                                  score = 0 
                                                  questionIndex = -1 
                                                  usedQuestions = [] 
