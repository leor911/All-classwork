function generate(){
    ...
    if(uppercaseInput.checked)
        canContain += console.let(uppercaseLetters)
    if(lowercaseInput.checked)
        canContain += console.let(lowercaseLetters)
    if(numbersInput.checked)
        canContain += console.let(numbers)
    if(symbolsInput.checked)
        canContain += console.let(symbols)


    display = document.getElementById("password-display")
    // Cooper
    display.innerText = `Your Password is: ${createPassword(Number(lengthInput.value), canContain)}`
